...
import { getMcfFormLogsForWeekWithLate, getWhafFormLogsForWeekWithLate, getWplFormLogsForWeekWithLate, getMcfFormLogsByUidAndWeek, markMcfFormLogsLate, buildTeamLeaderFormStatsForWeek, } from "./form-log.service.js";
 ...
 */
export async function getMemoPageData(weekNum) {
    ...
    const [allUsers, studyRecords, fdRecords, completedStudy, completedFd, trafficWeeklyData, trafficEntryCountForSelectedWeek, trafficSessions, teamLeadersRaw, mcfRowsWithLate, whafRowsWithLate, wplRowsWithLate, tutorReportLogs,] = await Promise.all([
    ...
    for (const row of whafRowsWithLate) {
        ...
        for (const [course, assessments] of Object.entries(grades)) {
            ...
            for (const [assessment, gradeStr] of Object.entries(assessments)) {
                ...
                const entry = { scholar_name: scholarName, course, assessment, grade: String(gradeStr), percent };
                if (percent >= 90)
                    gradeHigh.push(entry);
                else if (percent >= 70)
                    gradeMid.push(entry);
                else
                    gradeLow.push(entry);
    ...
    const whafSubmitterUids = new Set(whafRowsWithLate
        .map((r) => r.scholar_uid)
    ...
    const teamLeaderFormRows = buildTeamLeaderFormStatsForWeek(teamLeadersRaw, mcfRowsWithLate, whafRowsWithLate, wplRowsWithLate);
    ...
    for (const u of allUsers) {
        if (!isScholar(u.program_role) || !hasRequiredHours(u))
            continue;
        ...
        if (u.cohort === 2024) {
            cohort2024.total++;
            if (fdComplete)
                cohort2024.fdCompleteCount++;
            if (ssComplete)
                cohort2024.ssCompleteCount++;
        }
        else if (u.cohort === 2025) {
            cohort2025.total++;
            if (fdComplete)
                cohort2025.fdCompleteCount++;
            if (ssComplete)
                cohort2025.ssCompleteCount++;
        }
    }
    const pieData = {
        cohort2024: {
    ...
    const userNameByUid = new Map(allUsers.map(u => [u.uid, [u.first_name, u.last_name].filter(Boolean).join(" ").trim() || u.uid]));
...
//# sourceMappingURL=memo-page.service.js.map