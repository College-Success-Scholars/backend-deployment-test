...
import { FALL_SEMESTER_FIRST_DAY, WINTER_BREAK_FIRST_DAY, WINTER_BREAK_LAST_DAY, } from "../models/time.model.js";
import { createCampusCalendar } from "../../../shared/campus-calendar.js";
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
...
function getEasternDateParts(d) {
    const formatter = new Intl.DateTimeFormat("en-US", {
        timeZone: EASTERN_TIMEZONE,
        year: "numeric",
        month: "2-digit",
...
function easternCalendarDaysBetween(earlier, later) {
...
// ---------------------------------------------------------------------------
export function getEasternDayOfWeek(d) {
    const formatter = new Intl.DateTimeFormat("en-US", {
        timeZone: EASTERN_TIMEZONE,
        weekday: "short",
    });
...
export function getStartOfDayEastern(d) {
...
export const WINTER_BREAK_CAMPUS_WEEK_NUMBER = campusCalendar.weekOf(WINTER_BREAK_FIRST_DAY) ?? 0;
const WINTER_START = campusCalendar.rangeOf(WINTER_BREAK_CAMPUS_WEEK_NUMBER)?.startDate ??
    parseEasternDate(WINTER_BREAK_FIRST_DAY);
const WINTER_END = campusCalendar.rangeOf(WINTER_BREAK_CAMPUS_WEEK_NUMBER)?.endDate ??
    parseEasternDate(WINTER_BREAK_LAST_DAY);
const FIRST_SPRING_MONDAY = campusCalendar.rangeOf(WINTER_BREAK_CAMPUS_WEEK_NUMBER + 1)?.startDate ??
...
export function campusWeekToDateRange(weekNumber) {
...
export function dateToCampusWeek(date) {
    return campusCalendar.weekOf(date);
}
// ---------------------------------------------------------------------------
// Week fetch boundary helper
// ---------------------------------------------------------------------------
export function getWeekFetchEnd(range) {
    return new Date(range.endDate.getTime() + 24 * 60 * 60 * 1000 - 1);
}
// ---------------------------------------------------------------------------
// Display utilities
// ---------------------------------------------------------------------------
export function formatEntryDate(iso, showTime = false) {
    ...
    const dayNum = d2 ?? 1;
    const ord = dayNum === 1 || dayNum === 21 || dayNum === 31
        ? "st"
        : dayNum === 2 || dayNum === 22
            ? "nd"
            : dayNum === 3 || dayNum === 23
...
// ---------------------------------------------------------------------------
export function getCampusWeekForIsoWeek(isoWeek, currentIsoWeek) {
    const now = new Date();
