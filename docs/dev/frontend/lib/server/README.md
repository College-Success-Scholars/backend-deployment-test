...
**Location:** [`frontend/lib/server/`](../../../../../frontend/lib/server/)  
...
|------|-------------|-------------|
| `api-client.ts` | [source](../../../../../frontend/lib/server/api-client.ts) | Backend HTTP client — reads JWT from auth cookies, calls backend `/api/*` endpoints, unwraps `{ data }` responses. Exports: `backendFetch`, `backendGet`, `backendPost`, `backendPatch` |
