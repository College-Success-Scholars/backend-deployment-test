...
export default async function FormLogsTestPage({ searchParams }: PageProps) {
  ...
  const overall = teamLeaderRows.reduce(
    (acc, row) => ({
      ...
      mcfCompleted: acc.mcfCompleted + Math.min(row.mcfCompleted, Math.max(0, row.mcfRequired)),
      mcfRequired: acc.mcfRequired + Math.max(0, row.mcfRequired),
      mcfLateCount: acc.mcfLateCount + (row.mcfRequired > 0 && row.mcfLate ? 1 : 0),
